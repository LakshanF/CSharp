...
//        public ITaskItem[] ILCompilerPacks { get; set; }
        public ITaskItem[] ILCompilerRuntimePacks { get; set; }

        [Output]
        public ITaskItem[] ILCompilerHostPacks { get; set; }
            ...
            bool wasReferencedDirectly)
        {
            ...
            System.IO.File.AppendAllText(@"C:\Work\Core\Test\NativeAOT\8_10\sdk_clean\temp\SDK_Debug.txt", $"Line_499_{Laks_Debug++}_{runtimePackRuntimeIdentifier}{Environment.NewLine}");
        ...
        private bool AddAotOrR2RRuntimePackage(AotPackageType packageType, Version normalizedTargetFrameworkVersion, List<ITaskItem> packagesToDownload)
        {
            ...
            System.IO.File.AppendAllText(@"C:\Work\Core\Test\NativeAOT\8_10\sdk_clean\temp\SDK_Debug.txt", $"Line_600_{Laks_Debug++}_{hostRuntimeIdentifier}{Environment.NewLine}");
            ...
            System.IO.File.AppendAllText(@"C:\Work\Core\Test\NativeAOT\8_10\sdk_clean\temp\SDK_Debug.txt", $"Line_602_{Laks_Debug++}_NETCoreSdkRuntimeIdentifier:<{NETCoreSdkRuntimeIdentifier}>,hostRuntimeIdentifier:<{hostRuntimeIdentifier}>{Environment.NewLine}");
            System.IO.File.AppendAllText(@"C:\Work\Core\Test\NativeAOT\8_10\sdk_clean\temp\SDK_Debug.txt", $"Line_604_{Laks_Debug++},RuntimeIdentifier:<{RuntimeIdentifier}>, targetsRuntimeIdentifier:<{targetsRuntimeIdentifier}>{Environment.NewLine}");

            // ILCompiler supports cross target compilation
            //string runtimeIdentifierToReplace = (packageType == AotPackageType.ILCompiler && !hostRuntimeIdentifier.Equals(targetsRuntimeIdentifier)) ? targetsRuntimeIdentifier : hostRuntimeIdentifier;
            //var runtimePackName = packPattern.Replace("**RID**", runtimeIdentifierToReplace);
            var runtimePackName = packPattern.Replace("**RID**", hostRuntimeIdentifier);
            //var runtimePackName = "runtime.win-arm64.Microsoft.DotNet.ILCompiler";
            ...
            {
                ILCompilerHostPacks = new[] { newItem };
                // ILCompiler: If there is a cross-target request, we need to download that package as well
                if (packageType == AotPackageType.ILCompiler && !hostRuntimeIdentifier.Equals(targetsRuntimeIdentifier))
                {
                    ...
                    newItem.SetMetadata(MetadataKeys.NuGetPackageId, runtimeIlcPackName);
                    newItem.SetMetadata(MetadataKeys.NuGetPackageVersion, packVersion);
                    ILCompilerRuntimePacks = new[] { newItem2 };
                }
                else
                {
                    ILCompilerRuntimePacks = new[] { newItem };
                }
