...
namespace ManagedDriverNS
{
    ...
    public class ManagedDriver
    {
        ...
        [DllImport("NativeLibrary2", CallingConvention = CallingConvention.StdCall)]
        private static extern int addInLib2(int a, int b);

        public static int Main()
        {
            Console.WriteLine("LaksManagedEventSource: Waiting 10 seconds to client to get the PID");
            Thread.Sleep(10*1000);

            Console.WriteLine($"addInLib1:{addInLib1(2, 3)}");
            Console.WriteLine($"addInLib2:{addInLib2(12, 13)}");
            ...
            for (int i = 0; i < 100000; i++)
            {
                list.Add(new Foo(i));
                if (i % 10000 == 0)
                    Console.WriteLine($"LaksManagedEventSource: Fired Managed MyEvent {i:N0}/100,000 times...");
