...
def includeEvent_laks(inclusionList, providerName, eventName):
    if len(inclusionList) == 0:
        return True
    if eventName in inclusionList:
        return True
    else:
        return False
...
def generateClrEventPipeWriteEventsImpl(
    ...
    # WriteEventImpl.append("LAKS Include file" + str(inclusionList) + " " + providerName)
    # for eventNode in eventNodes:
    #     eventName = eventNode.getAttribute('symbol')

    #     if not includeEvent_laks(inclusionList_laks, providerName, eventName):
    #         continue
